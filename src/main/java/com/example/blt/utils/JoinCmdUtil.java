    ...
    public static String joinNewCmd(String type, String x, String y, Integer groupId, Integer sceneId) throws Exception {
        ...
        switch (typeEnum) {
            case MESH:
                if (StringUtils.isNotBlank(x)) {
                    sb = new StringBuffer();
                    cmd = sb.append(CmdJoin.CMD_MESH_START.getKey()).append(x).append(y).append(CmdJoin.CMD_MESH_END.getKey()).toString();
                }
                break;
            case GROUP:
                sb = new StringBuffer();
                String hexGroupId = String.format("%02x", groupId).toUpperCase();
                cmd = sb.append(CmdJoin.CMD_GROUP_START.getKey()).append(x).append(y)
                        .append(CmdJoin.CMD_GROUP_BETWEEN.getKey()).append(hexGroupId).append(CmdJoin.END.getKey()).toString();
                break;
            case SCENE:
                sb = new StringBuffer();
                String hexSceneId = String.format("%02x", sceneId).toUpperCase();
                cmd = sb.append(CmdJoin.CMD_SCENE_START).append(hexSceneId).append(CmdJoin.END).toString();
                break;
            default:
                throw new Exception("参数错误");
        }
