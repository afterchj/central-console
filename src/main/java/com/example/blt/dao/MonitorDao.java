...
public interface MonitorDao {
    ...
    @Select("select  lmac ,mname,lname,#{status} as status,d.groupId from f_light_demo d where mname like '%楼' ORDER " +
            "BY  mname,lname")
    List<LightDemo> getMonitorFromRemoteByStatus(@Param("status") String status);

    @Select("select  lmac ,mname,lname,#{status} as status,d.groupId from f_light_demo d where mname like '%楼' and" +
