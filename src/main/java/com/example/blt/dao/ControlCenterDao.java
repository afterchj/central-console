...
public interface ControlCenterDao {
    ...
    @Select("select if(status='1','在线','离线') AS state,ifnull( other, ifnull(mac,ip) ) AS pname, ifnull(mac,ip) as mac from t_host_info where mesh_id=#{meshId} and ip!='127.0.0.1'")
