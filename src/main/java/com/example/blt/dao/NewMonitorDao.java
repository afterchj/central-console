...
public interface NewMonitorDao {
    ...
    @Select("select Place from f_light_demo  where other='intelligence' and mname=#{floor} Group by substring_index(mname,'楼',1)+0,Place")
    List<PlaceWeb> getPlacesByFloor(@Param("floor")String floor);

    @Select("select Place,count(*) as exception from f_light_demo d left join t_light_info_copy i on d.lmac=i.lmac where d.other='intelligence' and i.y is null and i.status is null and d.mname=#{floor} Group by substring_index(mname,'楼',1)+0,Place")
