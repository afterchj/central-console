...
public interface LightListDao {
    ...
    @Select("SELECT d.lmac ,d.mname,d.lname ,CASE WHEN i.y = '32' THEN '1' when i.y is null and i.status ='0' then '1' when i.y is null and i.status ='1' then '0' WHEN i.y != '32' and i.y!=null THEN '0' when i.y is null and i.status is null then null  END AS status,d.Group  FROM f_light_demo d LEFT JOIN (select  lmac,x ,y,status from t_light_info Group by lmac) i ON d.lmac = i.lmac where d.mname='Office' ORDER BY d.mname,lname")
