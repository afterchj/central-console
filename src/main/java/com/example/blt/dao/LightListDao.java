...
public interface LightListDao {
    ...
    @Select("SELECT d.lmac,d.mname,d.lname  ,CASE WHEN i.x = '32' AND i.y = '32' THEN '1' WHEN i.x is not NULL AND ( i.x != '32' OR i.y != '32' ) THEN '0' ELSE NULL END AS STATUS,d.groupId FROM f_light_demo d LEFT JOIN (select  lmac,x ,y from t_light_info Group by lmac) i ON d.lmac = i.lmac ORDER BY d.mname,lname")//1:关 0：开 null 空
    List<LightDemo> getLightInfo();

    @Select("SELECT d.lmac ,d.mname,d.lname ,CASE WHEN i.y = '32' THEN '1' WHEN i.y != '32' and i.y is not null THEN '0' when i.y is null and i.status ='0' then '1' when i.y is null and i.status ='1' then '0'  when i.y is null and i.status is null then null  END AS status,d.groupId  FROM f_light_demo d LEFT JOIN (select  lmac,x ,y,status from t_light_info Group by lmac) i ON d.lmac = i.lmac where d.mname='Office' ORDER BY d.mname,lname")
...
//    List<LightDemo> getExhibitionLightInfo();
    ...
    @Select("SELECT d.lmac ,d.mname,d.lname ,CASE WHEN i.y = '32' THEN '1' WHEN i.y != '32' and i.y is not null THEN '0'  when i.y is null and i.status is null then null  END AS status,d.groupId  FROM f_light_demo d LEFT JOIN (select  lmac,x ,y,status from t_light_info Group by lmac) i ON d.lmac = i.lmac where d.mname='exhibition' ORDER BY d.mname,lname")
    List<LightDemo> getExhibitionLightInfo();
...
//    List<Map<String,Object>> getExhibitionCenterLNum();//每一楼灯个数
    ...
    @Select("SELECT d.lmac ,d.mname,d.lname ,CASE WHEN i.y = '32' THEN '1' WHEN i.y != '32'  THEN '0'  END AS status,d.groupId FROM f_light_demo d JOIN (select  lmac,x ,y,status from t_light_info where y is not null Group by lmac) i ON d.lmac = i.lmac where d.mname='exhibition' ORDER BY d.mname,lname")
